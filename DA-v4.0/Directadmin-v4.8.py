...
import requests, base64  # info decode
import time, concurrent.futures, getpass, html ## sleep / multithread / password / html entity decode 
import os # cmd line exit when check not match 

from colors import paint, warn # colorful text
from datetime import datetime  # time for logging 
from checker import Checker    # ASN check
from ssh import sshSession     # SSH session
...
paint("Ver 4.8\n","f_cyan")+
paint("    + Fixing HTML entity in message view and others\n","f_green",True)+
paint("    + Add some mail log defined\n","f_green",True)+
paint("    + Fix typo error in re-login\n","f_green",True)
             )
...
def printTickets(site):
    ...
    for mess in range(int(r.json()["messages"]["info"]["rows"]) - 1, -1, -1):
        ...
        row = html.unescape(row) # Convert HTML entity into unicode 
...
def mailQueue(site):
    ...
    loopError = {"#01": "OverQuotaTemp",
                 ...
                 "#04" : "Recipient address rejected: Greylisted",
    ...
    for mess in range(int(r.json()["table"]["info"]["rows"])):
        if not "<>" in r.json()["table"][str(mess)]["sender"]:
            ...
            for i in tempLogList:
                success = False
                if 'Received' in i or 'succeeded' in i or 'all complete' in i or i == "":
                    success = True
                if not success:
                    logList.append(i)
...
try:
    ...
    if debug:
        ...
        site = sites[10]
        r = br.get(site + actions['tickets'] + '?json=yes', headers=mkHeader(site))
        if r.json()["success"] == "no":
            log(r.json()["error"], site)
            print(paint("=Logged out due to unexpected error","f_red"))
            logged[site] = False
            print('FALSE')
        else :
